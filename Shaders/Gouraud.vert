...
uniform bool u_light_Test_GS = false;
uniform bool u_view_normal = false;
...
vec4 lighting_equation(in vec3 P_EC, in vec3 N_EC, in vec4 base_color) {
	...
	for (int i = 0; i < NUMBER_OF_LIGHTS_SUPPORTED; i++) {
		...
		if (u_light[i].position.w != zero_f) { // point light source
			...
			if (u_light[i].spot_cutoff_angle < 180.0f) { // [0.0f, 90.0f] or 180.0f
				...
				if (tmp_float >= cos(radians(spot_cutoff_angle))) {
					...
					if(i == 0 && u_view_normal) {
						color_sum = vec4(abs(N_EC), 1.0f);
						break;
					}
				}
				else 
		...
		if(local_scale_factor <= zero_f || tmp_float <= zero_f) num++;
	}
 	return color_sum;
}
...
void main(void) {
	...
	v_shaded_color = lighting_equation(position_EC, normal_EC, u_material.diffuse_color);
	if(num == NUMBER_OF_LIGHTS_SUPPORTED && u_light_Test_GS) v_shaded_color = vec4(vec3(zero_f), 0.0f);
